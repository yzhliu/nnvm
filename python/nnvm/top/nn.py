...
def compute_conv2d_prepack(attrs, inputs, _):
    """Compute definition of conv2d"""
    padding = attrs.get_int_tuple("padding")
    strides = attrs.get_int_tuple("strides")
    dilation = attrs.get_int_tuple("dilation")
    ...
    assert dilation == (1, 1), "not support dilate now"
    if groups == 1:
        out = topi.nn.conv2d_prepack(inputs[0], inputs[1], (channels, kh, kw), strides, padding)
    else:
...
def schedule_conv2d_prepack(attrs, outs, target):
    """Schedule definition of conv2d"""
    groups = attrs.get_int("groups")
    kh, kw = attrs.get_int_tuple('kernel_size')
    ...
    with tvm.target.create(target):
        if groups == 1:
            return topi.generic.schedule_conv2d_prepack((oc, kh, kw), outs)
        else:
...
# convolution no pack
@reg.register_compute("conv2d_nopack")
def compute_conv2d_nopack(attrs, inputs, _):
    """Compute definition of conv2d nopack"""
    padding = attrs.get_int_tuple("padding")
    strides = attrs.get_int_tuple("strides")
    dilation = attrs.get_int_tuple("dilation")
    kh, kw = attrs.get_int_tuple('kernel_size')
    ...
    channels = attrs.get_int("channels")
    layout = attrs["layout"]
    assert layout == "NCHWc", "only support nChwc for now"
    assert dilation == (1, 1), "not support dilate now"
    if groups == 1:
...
@reg.register_schedule("conv2d_nopack")
def schedule_conv2d_nopack(attrs, outs, target):
    """Schedule definition of conv2d"""
    groups = attrs.get_int("groups")
    kh, kw = attrs.get_int_tuple('kernel_size')
    oc = attrs.get_int("channels")
    with tvm.target.create(target):
        if groups == 1:
            return topi.generic.schedule_conv2d_nopack((oc, kh, kw), outs)
        else:
            raise ValueError("not support group number > 1 for now")

reg.register_pattern("conv2d_nopack", OpPattern.OUT_ELEMWISE_FUSABLE)
